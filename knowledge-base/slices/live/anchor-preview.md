...
主播预览是开播前的关键准备步骤。`LiveCoreView` 是专为直播场景设计的轻量级视图组件，通过 `viewType: .pushView` 模式将摄像头采集的本地画面渲染到屏幕上，让主播在正式开播前确认画面效果。
...
| 概念 / 方法 | 说明 |
|------------|------|
| `LiveCoreView(viewType: .pushView)` | 创建主播推流视图；`.pushView` 表示该视图用于采集和预览本地画面 |
| `setLiveID(_:)` | 将视图绑定到指定直播间 ID；**必须在开启摄像头之前调用** |
| `DeviceStore.shared.openLocalCamera(isFront:completion:)` | 打开本地摄像头并将预览帧渲染到已绑定的 `LiveCoreView` |
| `DeviceStore.shared.openLocalMicrophone(completion:)` | 打开本地麦克风，采集音频（预览阶段可选，开播前必须打开）|

### 预览 vs 推流

```
[预览阶段]  LiveCoreView(.pushView) + setLiveID + openLocalCamera
               ↓ 仅本地渲染，不推流
...
2. **先打开设备，再依赖预览** — 调用 `openLocalCamera` 的 `completion` 回调确认成功后，再进行 UI 更新（如移除加载遮罩）。
3. **提前申请系统权限** — 进入预览页面前通过 `AVCaptureDevice.authorizationStatus` 检查摄像头/麦克风权限，避免在 `openLocalCamera` 回调中才发现权限被拒导致用户体验差。
4. **将 LiveCoreView 正确加入视图层级** — `addSubview` 后设置好约束或 frame，否则视图不可见。
...
预览黑屏
├── setLiveID 是否已调用？
│   └─ 否 → 在 addSubview 之后、openLocalCamera 之前调用 setLiveID
├── openLocalCamera 是否成功（completion .success）？
│   ├─ .failure(-1101) → 摄像头权限被拒，引导用户去系统设置
│   ├─ .failure(-1102) → 摄像头被占用，提示关闭其他应用
│   └─ .failure(-1100) → 硬件失败，重试或上报
├── LiveCoreView 是否已 addSubview 并设置正确 frame/约束？
│   └─ 否 → 检查视图层级，确认视图可见
└── 模拟器环境？
    └─ 是 → 模拟器不支持摄像头，切换真机测试
