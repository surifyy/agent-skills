...
liveCoreView.joinLive(completion:)
liveCoreView.leaveLive(completion:)
```
...
5. **App 进入后台时清理播放资源** — iOS 系统在 App 后台时间过长后会回收媒体资源，应在 `didEnterBackground` 时停播，`willEnterForeground` 时重新进房或恢复。

### ❌ NEVER

1. **在 joinLive 成功前操作直播功能** — 弹幕、礼物、连麦等功能必须等待 `joinLive` 的 `.success` 回调，不可在进房请求未完成时调用。
...
4. **忽略进房失败** — 网络抖动、直播间已结束等原因均可导致 `joinLive` 失败，必须在 `.failure` 分支中处理并给用户反馈。
...
| 错误码 | 描述 | 处理建议 |
|--------|------|----------|
| `-1002` | 未登录 | 确保 `LoginStore.shared.login` 成功后再进房 |
...
├── joinLive 是否返回 .success？ → 否 → 查看错误码
│       ├─ -1002 → 先登录
│       ├─ -2001 → 直播已结束，返回列表
│       └─ -2003 → 先 leaveLive 再 joinLive
└── .success 但画面黑 → 检查 .playView 是否已添加到视图层级且 frame 非零

被踢出后没有提示
└── 确认已订阅 onKickedOutOfLive 并在回调中执行退出 + Toast

退出页面后内存/CPU 未下降
└── 确认 viewDidDisappear / deinit 中已调用 leaveLive
